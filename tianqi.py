...
def siliconflow_generate_forecast(amap_data):
    ...
    "messages": [
        {
            "role": "user",
            "content": f"content是高德地图返回的昆山市天气数据，根据数据播报明日天气，今天是{datetime.date}，语气要温柔可爱，发给女朋友的。返回的内容包括标点符号每行十个字以内,，总共六行，每一行严格按照后面我指定的内容描述。第一行描述白天和夜间天气现象，是否下雨；第二行描述白天和夜间气温；第三行描述风向风力；第四行描述穿衣建议和出行建议；第五行关心她，每天不重样；第六行夸赞女朋友的话，每天不重样。每一行前面不要有序号，不要有多余的换行符，不要有其他客气官方的语言。<content>{amap_data}</content>"
        }
    ],
    "stream": False,
    "max_tokens": 512,
